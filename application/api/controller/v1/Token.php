...
class Token
{
    ...
    public function token(Request $request)
    {
        ...
        //数据库已经有一个用户,这里需要根据input('carid')去数据库查找有没有这个用户
        $car = Car::get(['carid' => input('carid')]);
        $userInfo = [];
        if (!empty($car)) {
            $userInfo = [
                'uid' => $car['uid'],
    ...
    public static function checkParams($params = [])
    {
        ...
        $app = CarApp::get($params['appid']);
        if (empty($app)) {
            return self::returnMsg(401, 'appid 不存在');
        }
        self::$appsercet = $app['appsercet'];
